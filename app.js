...
bot.use(session()); // session mặc định (in-memory)

// THAY BẰNG ID TELEGRAM CỦA BẠN (Chiến)
const ADMIN_ID = '5589888565'; // Ví dụ: '123456789'
...
thêm function trong webhook để xóa tk đã bán trong danh sách /products
...
Lỗi confirm: WebhookError: Webhook validation failed: HTTP 400, Webhook url invalid (code: 20)
```<|control12|>Dựa trên ảnh dashboard PayOS bạn gửi và log Render, webhook URL đang đúng là `https://banhangtelegram.onrender.com/payos-webhook`, nhưng PayOS vẫn báo lỗi "Webhook url invalid (code: 20)" khi confirm. Điều này có nghĩa PayOS không thể validate URL (có thể do server trả về không đúng 200 OK khi PayOS test POST, hoặc URL có vấn đề nhỏ như dấu / thừa, hoặc server Render cold start chậm).

### Sửa lỗi "Webhook url invalid" (code: 20)
Để fix, chúng ta sẽ **tạm bỏ verify signature** trong webhook (skipVerify: true) để PayOS chấp nhận URL (chỉ dùng cho test/confirm, sau bật lại để bảo mật). Sau khi confirm OK, PayOS sẽ gọi webhook thật khi thanh toán.

Thay **toàn bộ** phần webhook trong `app.js` bằng đoạn code dưới (đã thêm skipVerify và log debug):
...
app.post('/payos-webhook', async (req, res) => {
  console.log('[WEBHOOK DEBUG] Nhận request POST từ PayOS:', req.body);  // Log để kiểm tra request đến

  try {
    // Tạm bỏ verify signature để test/confirm (sau này xóa skipVerify để bảo mật)
    ...
    console.error('Webhook error:', err.message || err);
    res.status(200).send('OK'); // Luôn trả 200 để PayOS chấp nhận URL khi test
  }
});
