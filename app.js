...
// ADMIN upload file TXT và xử lý button chọn loại (giữ nguyên code cũ của bạn)
...
app.post('/payos-webhook', async (req, res) => {
  try {
    ...
    if (webhookData.success) {
      ...
      if (order.type === 'account') {
        ...
        if (account) {
          ...
          console.log(`[WEBHOOK] Đã gửi tài khoản và xóa khỏi danh sách cho user ${order.userId}`);
        }
      } else {
    ...
    res.status(200).send('OK'); // Luôn trả 200 để PayOS chấp nhận
  }
});
